...
const TTL_DAYS = safeNumber(process.env.TICKER_CACHE_TTL_DAYS, 1);
const PROVIDER_MIN_INTERVAL_MS = safeNumber(process.env.ALPHA_VANTAGE_MIN_INTERVAL_MS, 13000);
...
app.get('/api/ticker/:symbol', async (req, res) => {
  ...
  if (!validation.valid) return res.status(400).json({ error: 'Invalid symbol' });

  if (!API_KEY) return res.status(500).json({ error: 'Missing ALPHA_VANTAGE_API_KEY in environment' });
  ...
  try {
    ...
    return res.status(error.statusCode || 500).json(error.payload || { error: error.message || 'Failed to fetch data' });
  }
});
...
app.post('/api/ticker/:symbol/refresh', async (req, res) => {
  ...
  try {
    ...
    return res.status(error.statusCode || 500).json(error.payload || { error: error.message || 'Failed to refresh data' });
  }
});
